...
class UserState:
    ...
    def userstate0(self):
        ...
        if guestprompt:
            ...
            #         st.markdown(body=guestprompt)
            #     self.userstate8_guestchat()
                
            # st.session_state.guestpromptid = OpenAI(api_key=st.secrets.openai.api_key).beta.threads.messages.create(role="user", content=guestprompt, thread_id=st.session_state.guestthreadid)
            # st.session_state.guestrun = OpenAI(api_key=st.secrets.openai.api_key).beta.threads.runs.create(thread_id=st.session_state.guestthreadid, assistant_id=st.secrets.openai.assistant_id)
            ...
            #     st.session_state.guestrun = OpenAI(api_key=st.secrets.openai.api_key).beta.threads.runs.retrieve(run_id=st.session_state.guestrun.id, thread_id=st.session_state.guestthreadid)
            #     if st.session_state.guestrun.status == "completed":
            #         st.toast("Complete")
            #         st.session_state.guestthreadmessages = OpenAI(api_key=st.secrets.openai.api_key).beta.threads.messages.list(thread_id=st.session_state.guestthreadid)
    ...
    def userstate2(self):
        st.session_state.checkoutsession = stripe.checkout.Session.create(api_key=st.se)
        st.info("You will be redirected to make a payment for SpartakusAI. Please click the button below to proceed.")
        btn = st.button(label="Button")
        if btn:
            self.userstate8_callback(userstate_number=3)
    
    def userstate3(self):
        btn2 = st.button(label="Button2")
        if btn2:
            self.userstate8_callback(userstate_number=4)
    ...
    def userstate4(self):
        ...
        submit_btn = st.button(label="Submit", type="primary")
        if submit_btn:
            self.userstate8_callback(userstate_number=5)
...
class SessionState:
    ...
    def initialize0_master(self):
        self.initialize1_userstate()
        self.intialize2_userinfo()
        self.initialize3_checkout()
        self.initialize8_guestchat()
        self.initialize9_lists()
        st.session_state.initailized = True

    def initialize1_userstate(self):
        st.session_state.userstate = None 
        st.session_state.usertype = None
    
    def intialize2_userinfo(self):
        st.session_state.userinfo = {"username": None, "password": None, "email": None, "businessname": None, "businessaddress": None, "firstname": None, "lastname": None, "fullname": None, "createddate": None, "threadid": None, "vectorstoreid": None, "userrole": None}
        st.session_state.username = None
        st.session_state.password = None
        st.session_state.email = None
    ...
    def initialize8_guestchat(self):
        st.session_state.guestthreadid = None
        st.session_state.guestpromptid = None
        st.session_state.guestresponse = None
        st.session_state.guestresponseid = None
